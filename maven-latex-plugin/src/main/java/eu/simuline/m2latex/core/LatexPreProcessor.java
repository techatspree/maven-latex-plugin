...
public class LatexPreProcessor extends AbstractLatexProcessor {
	 ...
	 */
	private final static Map<String, SuffixHandler> SUFFIX2HANDLER = new TreeMap<String, SuffixHandler>();
	...
	Collection<File> processGraphicsSelectMain(File dir, DirNode node)
		...
		if (!mainFilesIncluded.isEmpty()) {
			...
			if (!keySetOrg.containsAll(mainFilesIncluded)) {
				Set<String> includedNotMainFiles = new HashSet<String>(mainFilesIncluded);
