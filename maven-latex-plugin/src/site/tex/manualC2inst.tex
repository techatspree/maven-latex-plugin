...
Alternatively, the plugin can be configured also 
...
For all three goals a default phase is defined, 
...
It is perfectly ok to stick to a single format like PDF. 
...
cleanup may be deactivated and so the source directory may be polluted. 
Cleanup is recommended to make the individual runs of this plugin independent.
Thus, there is a dedicated goal \texttt{clr} 
...
signifying which may cause trouble 
...
\begin{table}[htb]
  ...
  \begin{tabular}{ll}
    ...
    \texttt{.latexmkrc}        & \href{\urlSite fromMain/.latexmkrc}{.latexmkrc} \\
    \texttt{.latexmkrc} (raw)  & \href{\urlSite fromMain/raw/.latexmkrc}{.latexmkrc} \\
