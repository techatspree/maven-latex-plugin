...
The last Section,~\ref{subsec:xmlPomInjections}, is to be eliminated. 
...
There are further situations where the user is grateful of being able to configure this software, 
...
\subsection{Injections}\label{subsec:xmlPomInjections}

This section must be rewritten: 
In the long run, here the goal \texttt{inj} is described 
and this is part of document development. 
Do not forget the command line usage with property. 
...
\begin{table}[htb]
  ...
  \begin{tabular}{lll}
    ...
    File & injection Link\\
    \midrule{}
    \texttt{instVScode4tex.sh} & \texttt{vscodeExt} & \href{\urlSite fromMain/instVScode4tex.sh}{instVScode4tex.sh} \\
    \texttt{.latexmkrc}        & \texttt{latexmkrc} & \href{\urlSite fromMain/.latexmkrc}{.latexmkrc} \\ % TBD: maybe new text 
    \texttt{.chktexrc}         & \texttt{chktexrc}  & \href{\urlSite fromMain/.chktexrc}{.chktexrc} \\
