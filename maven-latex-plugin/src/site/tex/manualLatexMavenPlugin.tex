...
\newcommand{\repo}{\texttt{https://www.simuline.eu/public\_html/RepositoryMaven}}
...
%
\begin{Verbatim}[fontsize=\small, commandchars=\\\{\}]
\repo/eu/simuline.m2latex
\end{Verbatim}
%
Moreover, ant expects the jar files in an according folder
to be found.
...
Then the declaration of the \texttt{path} 
and the two \texttt{taskdef}'s and targets
of a build file entry given in Listing~\ref{lst:fullConfigAnt}
on page \pageref{lst:fullConfigAnt} 
have to be added to the local \texttt{build.xml}.
Moreover, the two properties \texttt{antJarDir} and \texttt{createdJar}
must be adapted and added to the build file also. 
\index{ant-task}
