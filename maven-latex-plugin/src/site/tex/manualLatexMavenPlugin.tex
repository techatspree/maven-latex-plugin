...
\item
\texttt{fix-cm} described in \cite{FixCmP} and 
\texttt{anyfontsize} described in \cite{AnyfontsizeP} 
...
% FIXME: write BibTeX instead or, even better, bibtexCommand 
% because there are alternatives to bibtex 
...
for the post-processor \texttt{tex4ht} to use. 
Note that \texttt{tex4ht} is the name both of the post-processor 
...
and finally package `\texttt{tex4ht}' is for all the rest. 
...
emitted by the package `\texttt{rerunfilecheck}' 
...
\lstinputlisting[firstline=220, lastline=1108]{../../../pom.xml}% chktex 11 
