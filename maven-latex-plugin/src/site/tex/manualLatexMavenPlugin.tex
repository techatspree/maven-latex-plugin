...
via the command {\tt odt2doc} and thus require three steps 
...
the task definition {\tt latex} and the target {\tt latex:cfg}. 
...
among these are also {\tt dvipdfm} and {\tt dvipdfmx}. 
...
The file {\tt xxx.ptx} is ``inputted'' into the tex-file of this manual 
...
\subsection{The parameter {\tt patternLatexMainFile}}
...
\subsection{The parameter {\tt patternCreatedFromLatexMain}}
...
\subsection{The parameter {\tt metapostOptions}}
\label{subsec:metapostOptions}
TODO: add 
%%\tt\small -interaction=nonstopmode -recorder -s prologues=2

\subsection{The parameter {\tt svg2devOptions}}
...
\subsection{The parameter {\tt latex2pdfOptions}}
\label{subsec:latex2pdfOptions}
TODO: add 
%-interaction=nonstopmode -synctex=1 -src-specials -recorder -shell-escape 


\subsection{The parameter {\tt patternWarnLatex}}
\label{subsec:patternWarnLatex}
TODO: add 

\subsection{The parameter {\tt patternReRunLatex}}
...
\subsection{The parameter {\tt patternReRunMakeIndex}}
...
\begin{minipage}{0.95\linewidth}
The pattern in the `glg'-file 
which indicates an error when running `{\tt xindy}' 
...
\subsection{The parameter {\tt patternReRunMakeGlossaries}}
...
The explanation of the pattern `{\tt T\$T}' is the same as given 
...
\subsection{The parameter {\tt patternT4htOutputFiles}}
...
FIXME: 
Note that {\tt pdftotext -h} prints a usage message. 
This is a way to obtain not the specified output. 
It shows that {\tt pdftotext -q} does not print any messages or errors. 
This indicates that {\tt pdftotext} normally does display error messages 
on the standard output. 
These may be led to a log file to indicate errors and warnings. 
Here, further research is required. 

The option {\tt -htmlmeta} seems not approprate. 
The option resolution {\tt -r} seems sensible only in conjunction 
with the crop area defined by {\tt -x} and {\tt -y} 
which does not make sense in our context. 
The same holds for specification of the first and the last page 
via {\tt -f} and {\tt -l}. 
What does make sense is specifying the encoding via {\tt -enc} 
with possible values given by {\tt pdftotext -listenc}. 
What makes sense most ist {\tt UTF-8}. 
...
\item
method {\tt Runtime.exec(String, String[], File)} fails 
