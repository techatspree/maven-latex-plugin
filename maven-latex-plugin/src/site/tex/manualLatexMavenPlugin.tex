...
The default value 
'\textasciicircum T\$T\textbackslash.$[$\textasciicircum.$]*$' 
is appropriate for most parameters and packages. 
Package 'srcltx' 
requires in addition 
'\textasciicircum T\$T\textbackslash.synctex\textbackslash.gz' 
...
\begin{minipage}{0.95\linewidth}
The options for the command \$fig2devCommand 
specific for the output language `pdftex\_t'. 
Note that in addition to these options, 
the option `-L pdftex\_t' specifies the language, 
...
\begin{minipage}{0.95\linewidth}
The pattern in the log-file 
indicating a warning when running \$latex2pdfCommand. 
...
Nevertheless, `xindy' is invoked as `xindy -L english  -I xindy -M\ldots'. 
...
'\textasciicircum T\$T\textbackslash.yyy\$', where 'yyy' 
...
- '\textasciicircum T\$T\textbackslash.x?html?\$' is the main file. 
- '\textasciicircum T\$Tli\d+\textbackslash.x?html?\$' are lists: 
toc, lof, lot, indices, glossaries, NOT the bibliography. 
- '\textasciicircum T\$T(ch|se|su|ap)\textbackslash d+\textbackslash.x?html?\$' 
are chapters, sections and subsections or below 
and appendices. 
- '\textasciicircum T\$T\textbackslash d+\textbackslash.x?html?\$' are footnotes. 
- '\textasciicircum T\$T\textbackslash.css\$' are cascaded stylesheets. 
- '\textasciicircum T\$T-\textbackslash d+\textbackslash.svg\$' and 
  '\textasciicircum T\$T\textbackslash d+x\textbackslash.png\$' 
are svg/png-files representing figures. 
- '\textasciicircum(cmsy)\textbackslash d+(-c)?-\textbackslash d+c?\textbackslash.png\$' 
represents special symbols. 
		 
Note that the patterns for the html-files 
can be summarized as '\textasciicircum T\$T((ch|se|su|ap|li)?\d+)?\.x?html?\$'. 
...
{\tt org.apache.maven.plugin.MojoFailureException} 
if a recoverable error occurs and a 
{\tt org.apache.maven.plugin.MojoExecutionException} 
...
If this software throws an exception, 
as a maven plugin it throws a {\tt MojoFailureException} 
and as an ant task a {\tt BuildException}. 

Section~\ref{subsec:exception} indicates 
under what circumstances an exception is thrown 
and Section~\ref{subsec:logWarn} describes the warnings. 
