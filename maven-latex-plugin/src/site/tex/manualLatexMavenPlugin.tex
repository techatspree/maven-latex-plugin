...
\begin{itemize}
\item
\texttt{geometry} described in \cite{GeomP} 
to control page layout. 
\item
\texttt{microtype} described in \cite{MicroTyP} improve readability 
and make the document look nicer. 
It also helps to avoid bad boxes. 
\item
\texttt{hyperref} described in \cite{HyperTextP} 
to insert hypertext marks, which i do not want to miss in larger documents. 
\item
\texttt{srcltx} described in \cite{SrcLtxP} 
which allows to jump from the DVI file to the .tex source and back. 
\item
\texttt{showframe} 
if \texttt{geometry} is not used with option \texttt{showframe}. 
There seems to be no package documentation for package \texttt{showframe}. 
\item
\texttt{booktabs} described in \cite{BooktP} 
...
\begin{itemize}
\item
\texttt{rerunfilecheck} described in \cite{RerunFChkP} 
...
\item
\texttt{ifthen} described in \cite{IfThenP} 
which provides the \texttt{ifthenelse}-command 
which is needed to create both pdf and html and also to create rtf. 
\item
\texttt{ifpdf} described in \cite{IfPdfP} 
which provides the \texttt{ifpdf}-command to detect pdf-mode. 
...
in particular \texttt{graphicx}, \texttt{xcolor} and \texttt{transparent}, 
the latter two descibed in \cite{XColorP} and in \cite{TransP}, 
respectively. 
Sometimes also \texttt{bmpsize} described in \cite{BmpP} 
if pixel graphics is used. 
\item
\texttt{import} described in \cite{ImpoP} 
...
\begin{itemize}
\item
The ams-packages **** \texttt{amsmath}
\item
\texttt{longtable} described in \cite{LongTabP} 
for long tables, i.e.~tables exceeding a page. 
\item
\texttt{listings} described in \cite{ListingsP} for listings. 
\item
\texttt{fancyvrb} described in \cite{FancyVerbP} 
provides useful environments to mark verbatim text. 
\item
\texttt{ifxetex} described in \cite{IfXeTeXP} and 
\texttt{ifluatex} described in \cite{IfLuaTeXP} 
...
The goal \texttt{cfg} 
is executed in maven's \texttt{site} lifecycle by default. 
...
This is what the goal {\em graphics\/} accesible via 
...
\texttt{chktex} requires the \LaTeX-package \texttt{booktabs} 
...
with htlatex if invoking someting like \texttt{htlatex xxx.tex myxhtml,\ldots} 
...
Interesting enough, 
the commands \cmd{index}{} and \cmd{makeindex}{} 
...
Essentially, 
\cmd{makeindex}{} prepares writing to the \gls{idx}-file, 
whereas \cmd{index}{} performs writing a line. 
...
\cmd{printindex}{} which reads in the ind-file 
...
the command \cmd{gls}{} defined by the package \texttt{glossaries} 
...
(s)he is asked to contribute % chktex 36
...
If the user finds an extension, (s)he is asked to contribute % chktex 36
and to notify the developer of this plugin. 
...
indicating an error when running the command 
...
\begin{minipage}{0.95\linewidth}
The pattern in the log-file 
which indicates a warning when running the command 
\texttt{\$latex2pdfCommand}. 
...
which indicates an error when running the command 
\texttt{\$makeGlossariesCommand}. 
...
\lstinputlisting[firstline=220, lastline=1094]{../../../pom.xml}% chktex 11 
...
requires the following target and task definitions in \texttt{build.xml}. 

\lstset{language=xml, basicstyle=\scriptsize}% FIXME\@: nag complains for htlatex 
\lstinputlisting[firstline=74, lastline=238]{../../../build.xml}% chktex 11 
