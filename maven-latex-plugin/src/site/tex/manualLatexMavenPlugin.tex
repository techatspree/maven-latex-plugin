...
we have choosen an arbitrary one. 
...
- '\textasciicircum T\$T-\textbackslash d+\textbackslash.svg\$' 
...
Adding the patterns for the css-file and the svg-files, 
we obtain 
'\textasciicircum T\$T(((ch|se|su|ap|li)?\textbackslash
d+)?\textbackslash.x?html?|\textbackslash.css|-\textbackslash d+\textbackslash.svg)\$'. 
...
\lstinputlisting[firstline=203, lastline=788]{../../../pom.xml}
...
\lstset{language=xml, basicstyle=\scriptsize}
\lstinputlisting[firstline=80, lastline=168]{../../../build.xml}
...
The pattern for detection of an error requires option {\tt -no-file-line-error}
(which is the default) 
but does not work with option {\tt -file-line-error}. 
This yields 
%
\begin{verbatim}
./manualLatexMavenPlugin.tex:2484: Undefined control sequence.
l.2500 \bla
\end{verbatim}
%
instead of 
...
In target html, there seem to be created files {\tt$<$font$>$.png} 
and things like that. 
Neither {\tt mvn latex:html} copies them to the target folder 
nor {\tt mvn latex:clr} deletes them in the source folder. 
It seems more appropriate at least for target html, 
to specify the files to be deleted and the files to be copied 
through regular expressions, rather than by wildcards. 

For {\tt mvn latex:clr} maybe a new parameter would be helpful: 
suffixes to be ignored which also applies to creational targets, 
because preprocessing otherwise yields a warning 
