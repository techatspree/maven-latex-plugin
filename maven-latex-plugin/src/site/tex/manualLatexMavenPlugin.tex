...
As described in Section~\ref{subsec:rerunMakeIndexGlossaries} 
...
\section{Parameters resp. Settings}\label{sec:settings}
...
described in detail in Section~\ref{subsec:settingsGen}. 
...
with the parameters in Section~\ref{subsec:settingsLatex2Html}. 
...
\subsection{General Parameters}\label{subsec:settingsGen}
...
\tt patternClearFromLatexMain             & see below              \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
For goal `{\tt clr}', 
the tex source directory {\tt\$texSrcDirectory} is cleared, 
...
\subsection{Parameters for graphical preprocessing}\label{subsec:settingsGraph}
...
see {\tt\$fig2devPtxOptions} and {\tt\$fig2devPdfOptions}, 
respectively. 
...
\tt metapostOptions   & see below  \\%\tt\small -interaction=nonstopmode -recorder -s prologues=2
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\subsection{Parameters for the \LaTeX-to-pdf Conversion}
\label{subsec:settingsLatex2pdf}
...
\tt latex2pdfOptions   & see below  \\%\tt\small -interaction=nonstopmode -synctex=1 -src-specials -recorder -shell-escape 
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\subsection{Parameters for Creation of the Bibliography}
\label{subsec:settingsBibTeX}
...
\subsection{Parameters for Creation of the Index}
\label{subsec:settingsMakeIndex}
...
\tt patternReRunMakeIndex & see below         \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\subsection{Parameters for Creation of the Glossary}
\label{subsec:settingsMakeGlossaries}
...
\tt patternReRunMakeGlossaries  & see below         \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The pattern in the log file which triggers 
...
\subsection{Parameters for the \LaTeX-to-html Conversion}
\label{subsec:settingsLatex2Html}
...
\tt patternT4htOutputFiles         & see below              \\%-cvalidate
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
see {\tt\$patternClearFromLatexMain}. 
Spaces and newlines are removed 
from that pattern before processing. 
...
\subsection{Parameters for Conversion into further Formats}
\label{subsec:settingsExt}
...
\section{Exceptions and logging}\label{subsec:exceptionLogging}

A maven plugin may throw a import a 
%
\begin{verbatim}
org.apache.maven.plugin.MojoFailureException
\end{verbatim}
%
if a recoverable error occurs and a 
%
\begin{verbatim}
...
Whereas the former aborts the run of the plugin, 
the latter aborts the whole build. 
This maven plugin does not throw any {\tt MojoExecutionException} 
but only {\tt MojoFailureException}s. 
Thus not the whole build is aborted but only execution of the goal. 
FIXME: to be verified. 

An ant-task may throw a {\tt org.apache.tools.ant.BuildException} 
to abort the ant-build altogether. 
...
Section~\ref{subsec:exception} describes 
...
that the build system is out of control. 
Otherwise, a failure is indicated only by logging a warning, 
to allow the build process to go on, 
skipping a section of a task as small as possible. 


FIXME: not clear whether logging warnings is appropriate: 
Command line options: 
{\tt-e} shows error messages, 
{\tt-X} shows debug-messages, 
{\tt-q} quiet hides the info-level and shows {\em only} errors. 
There seems no way to get warnings only. 
Well, nowhere is described whether the goal is aborted 
when using an error. 


Section~\ref{subsec:logWarn} describes the warnings. 
There is also a lot of output for info and for debugging, 
but this is not described in this manual. 
...
This is described in detail in Section~\ref{subsec:exception} 
\item[E] logging as ERROR, which does not occur 
\item[W] logging as WARNING described in detail in Section~\ref{subsec:logWarn} 
\item[I] logging as INFO which occurs so frequently, 
that we do not document it here 
\item[D] logging as DEBUGGING output, also not described here. 
\end{itemize}
...
\subsection{Exceptions}\label{subsec:exception}
...
\hline
\hline
\tt TEX01 & \tt\scriptsize Error executing command '\$executable'.   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
Compare with WEX01 in Table~\ref{tab:WarnCEI}: 
...
\end{itemize}
%
whereas for WEX01 just a failure code is returned. 
\end{minipage}
} \\
\hline
\caption{\label{tab:TEX} The BuildFailureExceptions of CommandExecutorImpl  }
\end{longtable}
...
\hline
\hline
\tt TFU01 & \tt\scriptsize Required target directory '\$res' 
...
\tt TFU02 & \tt\scriptsize Error reading directory '\$texFileDir'!   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
FIXME: THIS SHALL BE ELIMINATED: REPLACED BY WFU01
\end{minipage}
} \\
\tt TFU03 & \tt\scriptsize Destination directory '\$targetDir'
		     cannot be created.   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\tt TFU04 & \tt\scriptsize Destination file '\$destFile' 
...
\tt TFU05  & \tt\scriptsize Destination file '\$destFile'
...
\tt TFU06 & \tt\scriptsize Error copying '\$srcFileName'
		     to '\$targetDir'.   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\tt TFU07 & \tt\scriptsize File '\$filePath' not found.  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
Does not occur, because exception is always caught. 
\end{minipage}
} \\
\tt TFU08 & \tt\scriptsize Error reading file '\$filePath'.  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
Does not occur, because exception is always caught. 
\end{minipage}
...
\hline
\hline
\tt TSS01  & \tt\scriptsize The tex source directory '\$texSrcDirectoryFile'\\
	   & \tt\scriptsize should be an existing directory, but is not.  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}

\end{minipage}
} \\
\tt TSS02  & \tt\scriptsize The output directory '\$outputDirectory' \\
           & \tt\scriptsize should be a directory if it exists, but is not.  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\subsection{Warnings}\label{subsec:logWarn}

A warning must be logged, if no exception is thrown 
but if there is an indication that the ant-task or the maven goal 
did not fully succeed. 
To avoid aborting the task or the goal, 
no errors are issued, warnings only. 

FIXME: this is not completely implemented. 
...
\hline
\hline
\tt WAP01  & \tt\scriptsize Running \$command failed. 
For details see \$logFileName.   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}

\end{minipage}
} \\
\tt WAP02 & \tt\scriptsize Running \$command failed: 
No log file '\$logFileName' found.   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}

\end{minipage}
} \\
\tt WAP03 & \tt\scriptsize Running \$command emitted warnings. 
For details see \$logFileName.   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\tt WAP04  & \tt\scriptsize Log file '\$logFile' is not readable: 
...
\hline
\caption{\label{tab:WarnALP} The Warnings of AbstractLatexProcessor ****  }
\end{longtable}
...
\hline
\hline
\tt WEX01  & \tt\scriptsize Executing '\$executable' 
...
\hline
\caption{\label{tab:WarnCEI} The Warnings of CommandLineExecutorImpl  }
\end{longtable}
...
\hline
\hline
\tt WPP01  & \tt\scriptsize Failed to delete file '\$file'. \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
FIXME: THIS SHALL BE ELIMINATED: REPLACED BY WFU05
\end{minipage}
} \\
\tt WPP02  & \tt\scriptsize File \$texFile' is not readable; 
...
\tt WPP03  & \tt\scriptsize Skipped processing of files 
...
\hline
\caption{\label{tab:WarnLPP} The Warnings of LatexPreProcessor ****  }
\end{longtable}
...
\hline
\hline
\tt WLP01  & \tt\scriptsize Maximum number \$maxNumReruns 
of LaTeX runs reached \\
 & \tt\scriptsize while another run is required.  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}

\end{minipage}
} \\
\tt WLP02  & \tt\scriptsize Log file '\$logFile' is unreadable: \\
	 & \tt\scriptsize Could not detect whether \$kind 
...
\tt WLP03   & \tt\scriptsize Running \$command created bad boxes.  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}

\end{minipage}
} \\
\tt WLP04  & \tt\scriptsize Running \$command emited warnings.   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\hline
\caption{\label{tab:WarnLP} The Warnings of LatexProcessor ****  }
\end{longtable}
...
\hline
\hline
\tt WFU01  & \tt\scriptsize Cannot read directory '\$dir'; 
...
\tt WFU02 & \tt\scriptsize LaTeX file '\$texFile' \\
          & \tt\scriptsize did not generate any output in '\$texFileDir'! \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
FIXME: THIS SHALL BE ELIMINATED
\end{minipage}
} \\
\tt WFU03  & \tt\scriptsize Cannot close '"\$closeable'.  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}

\end{minipage}
} \\
\tt WFU04  & \tt\scriptsize Cannot delete from directory '\$dir': \\
           & \tt\scriptsize is not readable.   \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
FIXME: THIS SHALL BE ELIMINATED: replaced by new WFU01. 
\end{minipage}
} \\
\tt WFU05  & \tt\scriptsize Failed to delete file '\$delFile'.  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\hline
\caption{\label{tab:WarnLP} The Warnings of TexFileUtilsImpl ****  }
\end{longtable}
...
\section{Listings}\label{subsec:listings}
...
\lstinputlisting[firstline=205, lastline=840]{../../../pom.xml}
...
\lstset{language=xml, basicstyle=\scriptsize}
\lstinputlisting[firstline=74, lastline=189]{../../../build.xml}
