...
dvi%
\fi.
The package \texttt{tex4ht} is \ifthenelse{\boolean{texFhtLoaded}}{}{not} loaded. 
...
The \LaTeX-files and also files included via \texttt{\textbackslash input} 
...
typically not inputted anywhere via \texttt{\textbackslash input}, 
...
\item
the second one via the command \texttt{\textbackslash includegraphics} 
...
``\texttt{\textbackslash includegraphics}'' etc.) in that directory. 
...
%
\lstset{language=tex, basicstyle=\small}% FIXME: nag complains for htlatex 
...
%
\lstset{language=tex, basicstyle=\scriptsize}% FIXME: nag complains for htlatex 
...
via \texttt{\textbackslash includegraphics\{xxx\}} (line 2). 
Note the following remarkable details: 
%
\begin{itemize}
\item
Observe that we can drop the suffix of the inlcuded file \texttt{xxx.pdf} 
...
we may use \texttt{\textbackslash input\{xxx.ptx\}} 
instead of \texttt{\textbackslash import\{yyy\}\{xxx.ptx\}}. 
...
\texttt{\textbackslash import\{yyy\}\{xxx.ptx\}}. 
...
Also the \texttt{\textbackslash import} command 
requires the \texttt{import} package. 
...
as \texttt{\textbackslash includegraphics\{xxx\}}. 
...
with the \texttt{\textbackslash input}-command. 
More than that, 
...
via \texttt{\textbackslash includegraphics\{xxx\}} 
...
and embedded in this \LaTeX-file 
with the \texttt{\textbackslash includegraphics}-command. 
...
via the \texttt{\textbackslash includegraphics}-command, 
...
\texttt{incscape} exports mixed pdf and latex if invoked approximately as 
%
...
and for gnuplot-files as described in Sections~\ref{sec:fig2dev} 
and \ref{sec:gnuplot2dev}. 
...
has no effect, i.e. inclusion uses the filename only. 
Thus \texttt{\textbackslash import} 
cannot be replaced by \texttt{\textbackslash input} 
and so the \LaTeX-package \texttt{import} is required. 
...
This is done as usual with the command \texttt{\textbackslash includegraphics}
provided by the package \texttt{graphicx}. 
According to the documentation \cite{GraX}, page 13, 
the bounding box must be provided somehow. 
This may be done via the package \texttt{bmpsize} 
...
Interesting enough, 
the commands \texttt{\textbackslash index} and \texttt{\textbackslash makeindex} 
...
Essentially, 
\texttt{\textbackslash makeindex} prepares writing to the \gls{idx}-file, 
whereas \texttt{\textbackslash index} performs writing a line. 
For example \texttt{\textbackslash index\{ant-task\}} creates an entry 
%
\begin{verbatim}
...
\texttt{\textbackslash printindex} which reads in the ind-file 
...
As for creating indices there is a \LaTeX-command \texttt{\textbackslash makeindex}, 
to create a glossary there is a \LaTeX-command 
\texttt{\textbackslash makeglossaries} 
but the latter is not builtin as \texttt{\textbackslash makeindex} 
but provided by the package \texttt{glossaries}. 
If \texttt{xxx.tex} is the \LaTeX{} main file, 
\texttt{\textbackslash makeglossaries} opens the glo-file \texttt{xxx.glo} 
containing glossary entries for writing. 
As the builtin command \texttt{\textbackslash index} 
writes entries into the idx-file defining the index, 
the command \texttt{\textbackslash gls} defined by the package \texttt{glossaries} 
...
\texttt{\textbackslash usepackage[makeindex]\{glossaries\}}. 
Using \texttt{xindy} instead of \texttt{makeindex} is triggered through 
\texttt{\textbackslash usepackage[xindy]\{glossaries\}}. 
...
\end{verbatim}
%
in particular before \texttt{\textbackslash makeindex} and 
\texttt{\textbackslash makeglossaries}. 
...
for the postprocessor \texttt{tex4ht} to use. 
Note that \texttt{tex4ht} is the name both of the postprocessor 
...
The special (\texttt{\textbackslash special}) 
...
through the ``\texttt{\textbackslash Needs\{\ldots\}}'' command.
...
\texttt{readTexSrcDirRec}  & \texttt{true}  \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
Whether the tex source directory \texttt{\$texSrcDirectory} 
...
\texttt{patternCreatedFromLatexMain} & see Section~\ref{subsec:patternCreatedFromLatexMain} \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
When clearing the tex source directory \texttt{\$texSrcDirectory}, 
...
containing \texttt{\textbackslash documentclass} for modern classes 
and \texttt{\textbackslash documentstyle} for old fashioned ones. 
This may be preceeded only by 
%
\begin{itemize}
\item
the command \texttt{\textbackslash RequirePackage}, 
\item
the command \texttt{\textbackslash PassOptionsToPackage}, 
by comment lines, 
\item
by \texttt{\textbackslash input}, and 
\item
by space. 
\end{itemize}
%
Note that \texttt{\textbackslash A} represents the beginning of the stream. 

Strictly speaking, also \texttt{\textbackslash(re)newcommands} 
and other constructs are possible but not allowed here. 
Strictly speaking also \texttt{\textbackslash documentclass} is not required, 
it could be hidden in some \texttt{\textbackslash input}-file. 
If we stick to the wise convention to open and close the same environment 
in the same file and to have \texttt{\textbackslash documentclass}, 
\texttt{\textbackslash begin\{document\}} and \texttt{\textbackslash end\{document\}} 
in the same file also, 
then a latex main file without \texttt{\textbackslash documentclass} 
cannot contain very much material. 

So we ask the user to have \texttt{\textbackslash documentclass} 
...
\texttt{metapostOptions} & see Section~\ref{subsec:patternCreatedFromLatexMain} \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\texttt{\textbackslash input\{latexfile.tex\}}. 
...
\texttt{patternErrLatex} & \texttt{(\^{}!\ )} \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The pattern in the log-file 
indicating a failure when running the \texttt{\$latex2pdfCommand}. 
%The default value is 
...
\begin{minipage}{0.95\linewidth}
The pattern in the log-file 
which indicates a warning when running \texttt{\$latex2pdfCommand}. 
...
To ensure termination, let \texttt{\$maxNumRerunsLatex} 
...
indicating that \texttt{\$bibtexCommand} failed. 
...
\begin{minipage}{0.95\linewidth}
The Pattern in the blg-file 
indicating a warning \texttt{\$bibtexCommand} emitted. 
The default value is chosen 
according to the `\texttt{bibtex}' documentation. 
\end{minipage}
} \\
...
\texttt{patternErrMakeIndex} & \texttt{(!! Input index error\ )}        \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The pattern in the ilg-file 
indicating that \texttt{\$makeIndexCommand} failed. 
The default value is chosen 
according to the `\texttt{makeindex}' documentation.
\end{minipage}
} \\
\texttt{patternWarnMakeIndex} & \texttt{(\#\# Warning )}        \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The pattern in the ilg-file 
indicating a warning \texttt{\$makeIndexCommand} emitted. 
The default value is chosen 
according to the `\texttt{makeindex}' documentation.
\end{minipage}
} \\
...
\texttt{(\^{}\textbackslash*\textbackslash*\textbackslash* unable to execute:\ )} \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The Pattern in the glg-file 
which indicates an error when running \texttt{\$makeGlossariesCommand}. 
If the default value is not appropriate, please modify 
and notify the developer of this plugin. 
\end{minipage}
} \\
\texttt{patternErrXindy} & \texttt{(\^{}ERROR:\ )}         \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The pattern in the `glg'-file 
...
\texttt{patternWarnXindy} & \texttt{(\^{}WARNING:\ )}         \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The pattern in the `glg'-file 
...
\texttt{patternT4htOutputFiles} & see Section~\ref{subsec:patternT4htOutputFiles}\\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
for the parameter \texttt{\$patternCreatedFromLatexMain}. 
Spaces and newlines are removed 
...
\begin{itemize}
\item
`\texttt{\^{}T\$T\textbackslash.x?html?\$}' is the main file. 
\item
...
\texttt{\footnotesize TSS02}  & \texttt{\footnotesize The output directory '\$outputDirectory'} \\
...
& \texttt{\footnotesize Target file '\$fileName' is not updated. } \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
FIXME
\end{minipage}
...
& \texttt{but maximum number \$maxNumRerunsLatex reached. } \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
FIXME
\end{minipage}
...
\lstset{language=xml, basicstyle=\scriptsize}% FIXME: nag complains for htlatex 
