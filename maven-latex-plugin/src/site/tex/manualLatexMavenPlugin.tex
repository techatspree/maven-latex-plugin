...
only a single glossary is supported by this software 
and also sorting and unifying is done via {\tt makeindex} 
as for indices, which is only one possible option. 

For generalizations see Section \ref{sec:gaps}. 

As for indices there is a command {\tt\textbackslash makeindex}, 
to create a glossary there is a command {\tt\textbackslash makeglossaries} 
...
This one uses the {\tt makeindex}-command as used for indices. 
It is the default but can also be activated through 
{\tt\textbackslash usepackage[makeindex]\{glossaries\}}. 
Let us mention that the third option using {\tt xindy} 
instead of {\tt makeindex} can be activated through 
{\tt\textbackslash usepackage[xindy]\{glossaries\}}. 


What the command {\tt\textbackslash makeglossaries} does is, 
writing the style definition by creating an ist-file {\tt xxx.ist} 
or an xdy-file {\tt xxx.xdy} 
if {\tt makeindex} or {\tt xindy} is specified as package option, 
respectively. 

The style {\tt list} (which is the default) 
is set in the form {\tt\textbackslash usepackage[style=list]\{glossaries\}}, 
...
currently is only supported using {\tt makeindex}. 
The options passed to {\tt makeindex} 
are those also specified for creating an index 
but supplemented with options {\tt -s}, {\tt -t}, {\tt -o}, to specify the
ist-file, the log-file (the transcript-file)
which has the ending {\tt glg} and the gls-file 
which is the result of sorting, the output file, 
and contains the entries of the glo-file 
just sorted, formatted and unified. 
This is illustrated in Figure \ref{fig:gloIst2gls}. 


\begin{figure}[htb]
\begin{center}
\input{12gloIst2gls.ptx}
\end{center}
\caption{\label{fig:gloIst2gls}Conversion of a glo-file into a gls-file 
using {\tt makeindex}}
\end{figure}

The mechanism to be supported in the long run as an alternative 
is illustrated in Figure \ref{fig:gloXdy2gls}. 

\begin{figure}[htb]
\begin{center}
\input{13gloXdy2gls.ptx}
\end{center}
\caption{\label{fig:gloXdy2gls}Conversion of a glo-file into a gls-file 
using {\tt xindy}}
\end{figure}
...
It must be checked whether the number of latex reruns suffices 
if a glossary is added (one latex run) 
and also added to the table of contents (another run?). 
