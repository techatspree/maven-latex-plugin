...
via option \texttt{-{}-output-format} as \texttt{lualatex} and \texttt{pdflatex}, 
or the alternative \gls{xdv} generalizing \gls{dvi} 
as \texttt{xelatex} does with the option \texttt{-{}-no-pdf}. 

In fact, the converter \texttt{latex2pdf} 
does much more than converting TEX files to PDF files. 
Figure~\ref{fig:tex2pdf} shows for \texttt{latex2pdf} set e.g.~to \texttt{lualatex}, 
...
The meaning of the term "`including"' used above % chktex 18
...
Running the \LaTeX-to-pdf converter on a file \texttt{xxx.tex} with package \pkg{pythontex} loaded 
yields a file \texttt{xxx.pytxcode} and if the package is loaded with option \texttt{depythontex} 
also a file \texttt{xxx.depytx}.
If the file \texttt{xxx.pytxcode} is present, 
this software invokes the command line tool \texttt{pythontex} (same name as the according package) 
to \texttt{xxx.pytxcode} (without ending) 
which converts this into a variety of output files, 
which are, without further configuration, all in the folder \texttt{pythontex-files-xxx}
as shown in Figure~\ref{fig:py2dir}, which is described in more detail in~\cite{PyTexInOut}, Section~3. 
...
An important remark is that \texttt{lualatex} is the preferred converter, 
because files \texttt{*.stdout} can impose heavy memory usage 
and currently \texttt{lualatex} is the only converter allocating memory dynamically. 

As one can see, \texttt{pythontex} cooperates with \texttt{lualatex} in a way 
...
Note that this is specific for unixoid operating systems but can be easily adapted to windows. 
...
Also, \texttt{depythontex} changes into interactive mode if the output file is already present. 
...
As \texttt{pythontex} also \texttt{depythontex} does not write a log file but just prints its errors and warnings. 
Thus, the author wrote a little wrapper, called \texttt{pythontexW} 
%
\begin{verbatim}
  #! /bin/bash
