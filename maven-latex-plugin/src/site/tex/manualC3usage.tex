...
Note that the pattern ends with the recommended \texttt{documentclass} 
or with the outdated \texttt{documentstyle}. 
...
For all document classes, it is recommended to load \pkg{nag} before 
using 
%
\begin{Verbatim}
...
The relations are configurable through setting \texttt{targets} and \texttt{docClassesToTargets} 
both in Table~\ref{tab:paramGen} on page~\pageref{tab:paramGen}. 
Also, if a document class occurs, which is not registered in \texttt{docClassesToTargets}, 
a warning \texttt{WLP09} described in Table~\ref{tab:WarnLPP} is displayed. % TBD: clarify whether this is not an exception 
...
by default the goal \texttt{inj} is tied to the phase \texttt{validate}, 
...
both tools are invoked directly by the user in the course of document development, 
...
The injection \texttt{header} given by \href{\urlSite fromTex/header.tex}{header.tex}, 
is tied to the file \texttt{header.tex} 
...
via forward search and backward search as described in Section~\ref{subsec:editViewLatex}. 
...
\begin{itemize}
  ...
  \item for creating PDF files but also other formats created with \pkg{tex4ht} 
\end{itemize}
...
Although there are alternatives like Emacs with extension AUC\TeX, 
...
and offers injection of an install script \texttt{instVScode4tex.sh} 
which allows installation of the relevant extensions of VS Code. 
\medskip


Besides the separation of editor and viewer the time delay between writing and reading 
...
and must be included with \texttt{\textbackslash include} 
not just input using \texttt{\textbackslash input}. 
With the command \texttt{\textbackslash includeonly} 
...
compiling a chapter with \texttt{\textbackslash includeonly} 
...
and then a build done by this plugin will pertain that file. 
For document development, 
the parameter \texttt{cleanUp} can be set to \texttt{false} 
...
This is more elegant than the idea of this plugin which is based on detecting hard coded set of files 
and tracing log files. 
If there are a lot of graphic files \texttt{latexmk} may be forced to run the latex generator frequently, 
...
After eliminating warnings until \texttt{chktex} displays no warnings, 
