...
and \texttt{beamer}, the latter of which is for presentations as described in~\cite{Beamer}. 
So far, this software has not been tested on KOMA\index{KOMA} classes 
which are treated in~\cite{KomaAnl}. 
Nevertheless, the pattern matches all possible document classes. 
Typically, the document class is loaded with options. 
Except for the \texttt{beamer} class, we suggest 
%
\begin{Verbatim}
...
It is possible to specify the language when loading \pkg{babel} as an option 
...
\lstinputlisting[linerange={113-137},
...
%
\lstinputlisting[linerange={97-99},
language=tex, basicstyle=\scriptsize, 
