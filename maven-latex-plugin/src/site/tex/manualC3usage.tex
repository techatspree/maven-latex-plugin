...
minimally the one given in Section~\ref{sec:xmlPom},
...
Listing~\ref{lst:chktexrc} shows the configuration \texttt{.chktexrc} 
needed to check this manual without error or warning. 
Details can be found in~\cite{ChkTeX22},~Section 6.1.5.  
...
Since it is too long to include it directly in this manual, 
we refer to its location on the website 
...
The installation script \texttt{instVScode4tex.sh} for VS Code extensions 
given in Listing~\ref{lst:instVScode} 
and the config files \texttt{.latexmkrc} and \texttt{.chktexrc} 
are packaged in the JAR file representing this software. 
In general all these files are filtered, 
i.e. they are defined using property names from the pom, but these are replaced by the actual value. 
For \texttt{.latexmkrc} the JAR file there is a raw variant with property names 
in subfolder \texttt{raw} which we refer to as the ``raw \texttt{.latexmkrc}''. 
...
Both \texttt{.latexmkrc} files and the file \texttt{.chktexrc} 
are intended as local configuration files. 


Listing~\ref{lst:depUnpack} shows the according configuration in the pom. 
In addition, the configuration files are available on the web as 
\href{\urlSite fromMain/.latexmkrc}{\texttt{.latexmkrc}}, 
\href{\urlSite fromMain/raw/.latexmkrc}{raw \texttt{.latexmkrc}} and as 
\href{\urlSite fromMain/.chktexrc}{\texttt{.chktexrc}}, respectively. 
...
\texttt{.chktexrc} is given in Listing~\ref{lst:chktexrc}. 
...
So far, the local configuration files are in just a single place. 
This is sufficient if using a command line option specifying its 
*****
