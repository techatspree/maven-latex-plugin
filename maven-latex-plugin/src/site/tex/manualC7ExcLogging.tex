...
\texttt{\footnotesize WFU10}  & \texttt{\footnotesize Cannot overwrite/clean config file '\$aFile' } \\
                              & \texttt{\footnotesize because it is not self-created. } \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
May occur if a config file \texttt{.latexmkrc} or \texttt{.chktexrc} is present in the latex source directory 
and is not created by this software. 
To avoid the risk of overwriting or deleting user-written config files, 
only config files written by this software can be overwritten in goal \texttt{dvl} 
or deleted in goal \texttt{clr}. 
\end{minipage} 
} \\
\texttt{\footnotesize WFU11}  & \texttt{\footnotesize Refuse to overwrite/clean config file '\$aFile' } \\
                              & \texttt{\footnotesize because it may be not self-created or has dangling reader.  } \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
To avoid the risk of overwriting or deleting user-written config files, 
this software checks whether it was this software which wrote the config files 
...
Neither is the file overwritten in goal \texttt{dvl} nor is it deleted in goal \texttt{clr}. 
\end{minipage}
}
\end{longtable}
