...
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The target set is given as a setting as described in Table~\ref{tab:paramGen}. 
The exception is thrown if a given target is unknown, e.g.\@ because it is misspelled. 
...
\texttt{\footnotesize TSS08}  & \texttt{\footnotesize Tried to use converter '\$convStr' } \\
 ...
 & \texttt{\footnotesize The target set '\$targetsStr' repeats target '\$target'. } \\
 \multicolumn2{|l|}{
 \begin{minipage}{0.95\linewidth}
 Indicates that in a target set a target is repeated. 
Sets contain elements only once. 
See also the Exception \texttt{TSS04} in this table. 
 %\texttt{\$target} in parameter \texttt{docClassesToTargets} 
% is not defined, e.g.\@ misspelled and must thus be ignored. 
 \end{minipage}
 } \\
...
%  \texttt{\footnotesize ESS01} % TSS11
%  & \texttt{\footnotesize Possible target '\$target' for document classes is unknown; ignoring. } \\
%  \multicolumn2{|l|}{
%  \begin{minipage}{0.95\linewidth}
%  Indicates that a possible target \texttt{\$target} in parameter \texttt{docClassesToTargets} 
%  is not defined, e.g.\@ misspelled and must thus be ignored. 
%  \end{minipage}
%  } \\
...
\end{minipage}
 ...
 \texttt{\footnotesize WSS02} 
 & \texttt{\footnotesize List '\$targets' repeats possible target for document classes. } \\
 \multicolumn2{|l|}{
 \begin{minipage}{0.95\linewidth}
 Indicates that the list of targets \texttt{\$targets} 
 in a chunk in parameter \texttt{docClassesToTargets} mentions one target more than once. 
 This merely reduces clarity.  
 \end{minipage}
 } \\
 \texttt{\footnotesize WSS03} 
 & \texttt{\footnotesize For document class '\$cls' specifying target set again; adding.  } \\
 \multicolumn2{|l|}{
 \begin{minipage}{0.95\linewidth}
 Indicates that in parameter \texttt{docClassesToTargets} a c;ass
 is syntactically not allowed. 
 Since it is intended to define allowed targets for document classes, 
 it is likely that some target intended by the user is not processed. 
 \end{minipage}
 } \\
...
\texttt{\footnotesize WLP09} & \texttt{\footnotesize For file '\$texFile' targets are not restricted } \\
                             & \texttt{\footnotesize by unknown document class '\$docClass'. } \\
 \multicolumn2{|l|}{
 \begin{minipage}{0.95\linewidth}
