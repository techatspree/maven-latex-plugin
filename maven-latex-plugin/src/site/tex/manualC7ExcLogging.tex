...
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
Indicates that a target in a target set given by setting \texttt{\$target} 
or in a chunk in setting \texttt{\$docClassesToTargets} is unknown, e.g.\@ because it is misspelled. 
...
\texttt{\footnotesize TSS08}  & \texttt{\footnotesize Tried to use converter '\$convStr' } \\
 ...
 & \texttt{\footnotesize The target set/chunk '\$targetsChunkStr' repeats target '\$target'. } \\
 \multicolumn2{|l|}{
 \begin{minipage}{0.95\linewidth}
 Indicates that a target in a target set given by setting \texttt{\$target} 
 or in a chunk in setting \texttt{\$docClassesToTargets} is repeated, 
 despite sets contain elements only once. 
