...
\begin{itemize}
  \item
  ` \texttt{article,book:chk,dvi,pdf,html,odt,docx,rtf,txt}'
  ensures that article and book allow all targets. 
  \item
  ` \texttt{beamer:chk,pdf,txt}' beamer allows mainly \texttt{pdf} and derived from that \texttt{txt}.  
...
\begin{longtable}{|ll|}
  ...
  \endfirsthead%
  \bottomrule
  \caption{\label{tab:paramMisc} Miscellaneous parameters}
  \endlastfoot%
...
This is a comma separated list of \texttt{injections}s without blanks. 
...
This pattern identifies files to be compiled, the so called \LaTeX{} main files, 
and contributes to finding the targets for which they are compiled. 

The default pattern for identifying a 
i.e. a file to be compiled 
and for
is given by Listing~\ref{lst:patternLatexMainFile}. 


\lstinputlisting[language=XML, basicstyle=\scriptsize,
linerange={166-173}, % chktex 8 TBD: why is this not covered by .chktexrc? 
breaklines,
%float,
...
The first line, the \texttt{\textbackslash A} indicates the start of the file. 

The most important line in this pattern is the last one. 
Taking its core alone, it means that a \LaTeX{} main file is one 
containing \cmd{documentclass} for modern documents 
and \cmd{documentstyle} for old-fashioned ones. 
This is followed by optional options and above all by a named capturing group \texttt{docClass} 
matching the document class. 

The next important line is the second one, describing a magic comment. 
A magic comment is a comment hiding information from one tool in a comment 
but exposing it to another tool 
Whereas comments start with \texttt\%, magic comments typically start with \texttt{\%!}, 
followed by (a blank with) an identifier for the tool they are significant for. 
In particular, the identifier for this \LaTeX{} builder is \texttt{LMP}, 
which is short for latex maven plugin, which is not entirely correct but intuitive. 
% TBD eliminate this duplicate. 
As the command \cmd{documentclass}, 
the magic comment can specify the document class with a capturing group 
named \texttt{docClassMagic}. 
It can also specify a target set specific for the document 
via the capturing group \texttt{docClassMagic}. 
As described in Section~\ref{subsec:sourcesLatex}, 
the pattern \texttt{patternLatexMainFile} 
must specify all these three capturing groups. 
Whereas in regular expressions group names are enclosed between \texttt< and \texttt>, 
these symbols must be escaped writing \texttt{\&lt;} and \texttt{\&gt;}, 
respectively. 


Besides the magic comments which are the start of the pattern, 
...
\item
comment lines even magic comments, although for this tool they are ignored. 
\item
\cmd{input}, and 
\item
space. 
...
One could use a convention, e.g.~marking main files with first line \texttt{\%~!MAIN FILE}, %TBD: eliminated: not typical for magic comments
...
Occurrence of \cmd{documentclass} and 
...
\section{Parameters for creation of the Gglossary}%
