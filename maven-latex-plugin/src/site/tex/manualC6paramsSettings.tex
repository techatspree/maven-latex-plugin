...
For example, \texttt{lualatex} is the default converter in the category \texttt{latex}. 
...
Note that for \texttt{xelatex} dvi mode 
...
the processor \texttt{xelatex} does not create \texttt{dvi} 
but \texttt{xdi} files. 
Thus, with \texttt{xelatex} the flag \texttt{\$pdfViaDvi} may not be set. 
...
Note that unlike the other engines, \texttt{lualatex} defines options starting with \texttt{-{}-}, 
...
\begin{description}
  ...
  \item[-no-pdf] generate XDV (extended DVI) output rather than PDF. 
   This is specific for \texttt{xelatex}. 
...
indicating that running \texttt{pythontex}, resp. \texttt{pythontexW} via \texttt{\$pythontexCommand} failed.
