...
Whereas the features implemented are outlined in the sequel [test](#output-in-various-formats)
in [Section 'Features'](#features) 
the features to come are collected in 
[Section 'Feature Requests'](#feature-requests).
...
we refer to ['The latex-maven-plugin' Section](#how-to-use-the-latex-maven-plugin)
and to ['The latex ant tasks' Section](#how-to-use-the-latex-ant-tasks), respectively. 
...
are collected as ['To-Dos'](#to-dos) without special ordering. 
...
Section [Features](#features) describes the features of the converter 
...
Finally, there is a section on the [developer](#to-the-plugin-developer). 
...
In the sequel the [supported output formats](#output-in-various-formats) 
are described. 
The focus is the **final** creation of output files from latex sources, 
but this software also supports the process to reach that final result, 
we refer to as [document development](#support-for-document-development). 

While creating the output files, also a lot of intermediate auxiliary files, 
as log files are created. 
For sake of reproducibility these must be cleaned up again. 
On the other hand, there are reasons to keep them. 
So the details of cleanup is devoted a separate [section](#cleanup-after-creation). 

Reproducibility of artifacts is another feature, realized for PDF output only. 
Thus, there is a mechanism to [check equality](#reproducibility-and-equality-artifacts) 
of the newly created document with the last one. 

Latex documents may contain bibliographies, indices, glossaries 
but also a table of contents and similar lists 
and [support](#inclusion-of-bibliographies-indices-glossaries-and-registers) 
of these elements is vital. 
More and more also inclusion of programs, 
as listings and even inclusion of computation result play a role. 
This '**LaTeX builder**' satisfies this need 
by supporting [`pythontex`](#inclusion-of-results-of-computations-and-listings). 

To ensure quality, 
this '**LaTeX builder**' [checks](#check-of-sources-and-results-logging) 
latex sources and log files. 

Although latex focuses on texts, it may also include graphics in various formats, 
but most of them need auxiliary programs to process 
before they can be inserted in the latex document. 
This '**LaTeX builder**' supports 
the necessary [graphics processing](#inclusion-of-various-graphics-formats). 

Typically, maven plugins and ant tasks can be implemented in pure java. 
In contrast, this **LaTeX builder**' just orchestrates 
invocation of various auxiliary programs and so depends on their compatibility. 
Thus, [version checks](#version-checks) are a means to ensure interoperability 
and if it is broken may give a hint on a possible reason. 
...
For further plans see [here](#feature-requests). 
...
[Section 'Cleanup after creation'](#cleanup-after-creation). 
...
For details see 
[Section 'Support for document development'](#support-for-document-development). 
...
In the [manual](https://www.simuline.eu/LatexMavenPlugin/manualLMP.pdf), 
...
but in an item of [Section 'Feature Request'](#feature-requests). 
...
More and more, documents become active. 
They may be on computer programs, contain listings 
and even include results of computations, in numeric, symbolic or graphic form. 
This '**LaTeX builder**' satisfies this need by supporting package and tool `pythontex`. 
Note that at time of this writing, `pythontex` supports programming in various languages, 
not only in python and new languages can be added easily. 

Unlike Lua language which is native only to `lualatex`, 
inclusion via `pythontex` is independent of the latex processor. 
...
Beyond the features presented in [Section 'Features'](#features), 
...
are given in Sections on the [latex-maven-plugin](#how-to-use-the-latex-maven-plugin) 
and the [latex ant tasks](#how-to-use-the-latex-ant-tasks) and their subsections. 
...
in the course of [document development](#support-for-document-development). 
...
in [Section 'Cleanup after creation'](#cleanup-after-creation). 
Invocation is e.g. as `mvn latex:cfg`. 

These goals are in a one-to-one relation 
to an according ant task 
described in [Section 'How to use the latex ant tasks'](#how-to-use-the-latex-ant-tasks).
...
We refer the reader to [Section 'Installation'](#installation) 
...
described in [Section 'The latex-maven-plugin'](#how-to-use-the-latex-maven-plugin) 
