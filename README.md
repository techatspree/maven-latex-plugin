...
and which provides its (common) features are described in the sequel. 
...
Also note that the manual of this software is written in LaTeX 
and can be generated by the maven plugin and by the ant task it refers to. 
So the [pom file](./maven-latex-plugin/pom.xml) 
and the [build file](./maven-latex-plugin/build.xml) 
also illustrate installation and useage. 
